import { useFormik } from "formik";
import { Link } from "react-router-dom";
import * as yup from "yup";
import axios from "axios";
import LoginInput from "./utilities/LoginInput";

type LoginDetails = {
  username: string;
  password: string;
  studentNumber: number;
  email: string;
  telephone: string;
};

const Register = () => {
  const initialValues: LoginDetails = {
    username: "",
    password: "",
    studentNumber: 0,
    email: "",
    telephone: "",
  };

  const LoginSchema = yup.object().shape({
    username: yup.string().required().label("Username"),
    password: yup.string().required().label("Password"),
    studentNumber: yup.number().required().label("Student number"),
    email: yup.string().email().required().label("Email"),
    telephone: yup.string().required().label("Password"),
  });

  const {
    handleBlur,
    handleChange,
    handleSubmit,
    errors,
    touched,
    isSubmitting,
  } = useFormik({
    initialValues: initialValues,
    onSubmit: async (values) => {
      const url = "http://127.0.0.1:8000/api/login";
      console.log("Values: ", values);
    },
    validationSchema: LoginSchema,
  });

  return (
    <>
      <div className="register_form border-2 border-green-700 bg-white rounded-lg p-5 mx-auto">
        <header className="flex justify-center items-center flex-col">
          {/* <img
        //    src={MukLogo}
           alt="logo"
           style={{ width: "200px", height: "200px" }}
           className="me-5 muk_logo"
           loading="lazy"
         /> */}
          <p className="text-green-700 font-semibold text-xl">Peps</p>
          {/* <p className="text-green-700 font-semibold">Teaching Load</p> */}
        </header>
        <div className="">
          <div className=" bg-white mt-2 rounded-md mb-5">
            <form className="" onSubmit={handleSubmit}>
              <h4 className="text-green-700 text-center text-xl font-semibold mb-5">
                Login into your account
              </h4>
              <div>
                <LoginInput
                  label="Username"
                  name="username"
                  type="text"
                  touched={touched}
                  errors={errors}
                  placeholder="Please enter your username here..."
                  handleChange={handleChange}
                  handleBlur={handleBlur}
                />
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <LoginInput
                    label="Student number"
                    name="studentNumber"
                    type="number"
                    touched={touched}
                    errors={errors}
                    placeholder="Please enter your student number here..."
                    handleChange={handleChange}
                    handleBlur={handleBlur}
                  />
                </div>
                <div>
                  <LoginInput
                    label="Telephone"
                    name="telephone"
                    type="text"
                    touched={touched}
                    errors={errors}
                    placeholder="Please enter your student number here..."
                    handleChange={handleChange}
                    handleBlur={handleBlur}
                  />
                </div>
              </div>
              <div>
                <LoginInput
                  label="Email"
                  name="email"
                  type="email"
                  touched={touched}
                  errors={errors}
                  placeholder="Please enter your username here..."
                  handleChange={handleChange}
                  handleBlur={handleBlur}
                />
              </div>
              <div>
                <LoginInput
                  label="Password"
                  name="password"
                  type="password"
                  touched={touched}
                  errors={errors}
                  placeholder="Please enter your password name.."
                  handleChange={handleChange}
                  handleBlur={handleBlur}
                />
              </div>

              <div className="flex justify-center items-center w-full">
                {isSubmitting ? (
                  <button
                    className="w-full text-white px-4 rounded py-2 bg-green-400 mt-2 "
                    type="submit"
                    disabled
                  >
                    Login
                  </button>
                ) : (
                  <button
                    className="w-full text-white px-4 rounded py-2 bg-green-700 mt-2 hover:scale-95"
                    type="submit"
                  >
                    Login
                  </button>
                )}
              </div>
            </form>
            <p className="mt-3 text-dark">
              Already have an account?{" "}
              <Link to="/">
                <span className="text-green-700">Login</span>
              </Link>{" "}
              here
            </p>
          </div>
        </div>
      </div>
    </>
  );
};

export default Register;
